...
namespace
{
    ...
    {
        return rclcpp::get_logger("robot_collision_checking.fcl_interface_collision_world");
    }
} // namespace

FCLInterfaceCollisionWorld::FCLInterfaceCollisionWorld(const std::string& frame_id) : frame_(frame_id), obj_counter_(0)
{
    fcl_collision_objects_.clear();
    RCLCPP_INFO(getLogger(), "Creating an FCL interface collision world");
}

FCLInterfaceCollisionWorld::~FCLInterfaceCollisionWorld()
{
    RCLCPP_INFO(getLogger(), "Destroying collision world");
}
            ...
            for (uint32_t y_grid = 0; y_grid < y_size; ++y_grid)
            {
                for (uint32_t z_grid = 0; z_grid < z_size; ++z_grid)
                {
                    ...
                    if (status == nav2_voxel_grid::MARKED)
                    {
                        ...
                        fcl::Transform3d world_to_fcl;
                        fcl_interface::transform2fcl(world_to_cell, world_to_fcl);
